...
def add_music_simple(video_path, music_url, output_path):
    ...
    try:
        ...
        if has_audio:
            cmd = ["ffmpeg", "-y", "-i", video_path, "-i", music_path, "-filter_complex", "[1:a]volume=0.25[a1];[0:a][a1]amix=inputs=2:duration=first[aout]", "-map", "0:v", "-map", "[aout]", "-c:v", "copy", "-c:a", "aac", output_path]
        else:
            cmd = ["ffmpeg", "-y", "-i", video_path, "-i", music_path, "-map", "0:v", "-map", "1:a", "-c:v", "copy", "-c:a", "aac", "-shortest", output_path]
...
def adjust_speed(input_path, output_path, speed_factor=0.5):
    cmd = ["ffmpeg", "-y", "-i", input_path,
           ...
           "-af", f"atempo={speed_factor}", output_path]
    subprocess.run(cmd, check=False, capture_output=True)
    return output_path
def add_text_simple(video_path, text, output_path):
